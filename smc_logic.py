...
def build_entry_sl_tp_from_smc(df_5m: pd.DataFrame,
    ...
    # IPC: lebih utamakan deep discount sebagai base, lalu moderate, lalu mid-range
    if in_disc_62_79:
    ...
    # IPC block priority: Mitigation / Breaker / FVG
    if mb_low and mb_high and mb_high > mb_low:
        ...
        # combine FVG mid dengan base discount
        entry = (base_entry + (fvg_low + fvg_high) / 2.0) / 2.0

    # Sedikit luas untuk SL biar masih protektif tapi ga terlalu sempit
...
# ============================================================
#                    ANALYZE SYMBOL (SMC IPC)
# ============================================================

def analyse_symbol(symbol: str):
    """
    Analisa 1 symbol dengan pendekatan SMC IPC (versi ketat, fokus akurasi):
    - HTF bias 1H: WAJIB strong bullish (EMA20 > EMA50 > EMA200)
    - Struktur 15m bullish (HH/HL)
    - Harga di area discount yang kuat:
        * Deep discount (ideal), atau
        * Discount moderat + nempel Mitigation/Breaker
    - Wajib ada IPC block utama (Mitigation / Breaker), FVG hanya pendukung
    - Wajib ada trigger CHoCH impuls
    - Wajib ada liquidity event berupa sweep
    - Filter momentum, fake pump, dan choppy
    - Return (conditions, levels) atau (None, None) jika setup IPC tidak valid
    """
    try:
    ...
    # ====== Filter keras: kondisi market tidak sehat ======
    if fake_pump or (not momentum_ok) or choppy:
        return None, None

    # ====== INTI LOGIC SMC IPC (VERSI KETAT) ======

    # 1. Bias & struktur (HTF)
    #   → sekarang WAJIB strong bullish, bukan sekadar "tidak bearish"
    bias_ok = bias_strong
    struct_ok = struct_15m

    # 2. Discount rules
    deep_discount = in62           # 0.62–0.80 → ideal
    moderate_discount = in50       # 0.40–0.62

    # IPC discount utama:
    #   - kalau deep discount → OK
    #   - kalau cuma moderate → harus nempel Mitigation / Breaker
    uses_mb_or_bb = mb or bb
    ipc_discount_core = deep_discount or (moderate_discount and uses_mb_or_bb)

    # 3. IPC block utama: WAJIB Mitigation / Breaker
    ipc_block_core = uses_mb_or_bb
    ipc_block_any = ipc_block_core or fvg  # fvg = extra confluence, bukan syarat wajib

    # 4. Trigger: WAJIB CHoCH impuls, pre-pump hanya pendukung
    ipc_trigger_core = choch
    ipc_trigger_support = pre_pump_ctx

    # 5. Liquidity event: WAJIB sweep; cluster likuiditas hanya pendukung
    ipc_liquidity_core = sweep
    ipc_liquidity_support = liq

    # All conditions untuk valid IPC setup (high accuracy / high winrate)
    ipc_valid_setup = (
        bias_ok and
        struct_ok and
        ipc_discount_core and
        ipc_block_core and
        ipc_trigger_core and
        ipc_liquidity_core
    )

    if not ipc_valid_setup:
        return None, None
    ...
    conditions = {
        ...
        "ipc_discount_core": ipc_discount_core,
        "ipc_deep_discount": deep_discount,

        # Blocks / FVG sebagai IPC
        "has_fvg_fresh": fvg,
        "has_mitigation_block": mb,
        "has_breaker_block": bb,
        "ipc_block_core": ipc_block_core,
        "ipc_block_any": ipc_block_any,

        # Market quality filters
        "ema_alignment_bullish": bias_strong,
        "momentum_ok": momentum_ok,
        "not_fake_pump": not fake_pump,
        "not_choppy": not choppy,
        "no_bearish_divergence": True,  # placeholder
        "no_exhaustion_sign": True,     # placeholder

        # IPC overall flag
        "ipc_trigger_core": ipc_trigger_core,
        "ipc_trigger_support": ipc_trigger_support,
        "ipc_liquidity_core": ipc_liquidity_core,
        "ipc_liquidity_support": ipc_liquidity_support,
        "ipc_valid_setup": ipc_valid_setup,
    }
