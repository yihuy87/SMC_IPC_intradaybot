...
def build_signal_message(
    ...
    # ====== Core flags (selaras dengan smc_logic IPC + smc_scoring) ======
    bias_strong = conditions.get("bias_1h_strong_bullish")
    bias_not_bear = conditions.get("bias_1h_not_bearish")
    bias_ok = bias_strong or bias_not_bear

    struct_ok = conditions.get("struct_15m_bullish")

    # Discount IPC
    disc_core = conditions.get("ipc_discount_core")
    deep_disc = conditions.get("ipc_deep_discount") or conditions.get("in_discount_62_79")
    mod_disc = conditions.get("in_discount_50_62")

    # Block IPC
    mb_ok = conditions.get("has_mitigation_block")
    brk_ok = conditions.get("has_breaker_block")
    block_core = conditions.get("ipc_block_core") or (mb_ok or brk_ok)
    fvg_ok = conditions.get("has_fvg_fresh")
    block_any = conditions.get("ipc_block_any") or block_core or fvg_ok

    # Trigger & Liquidity
    sweep_ok = conditions.get("ipc_liquidity_core") or conditions.get("has_big_sweep")
    choch_ok = conditions.get("ipc_trigger_core") or conditions.get("has_choch_impulse")
    liq_target_ok = conditions.get("ipc_liquidity_support") or conditions.get("liquidity_target_clear")

    # Market quality
    momentum_ok = conditions.get("momentum_ok")
    not_fake = conditions.get("not_fake_pump")
    not_choppy = conditions.get("not_choppy")
    market_ok = bool(momentum_ok and not_fake and not_choppy)

    # Context kompresi 70% (pre-pump)
    pre_pump = conditions.get("ipc_trigger_support") or conditions.get("has_pre_pump_context")
    if pre_pump:
        context_text = "Pre-pump kompresi 70% aktif — potensi impuls bullish (IPC context)."
    else:
        context_text = "Context normal (tanpa kompresi kuat, chain IPC tetap lengkap)."

    # Detail discount text
    if deep_disc:
        discount_text = "Deep discount IPC (62–79) — area ideal."
    elif disc_core and mod_disc:
        discount_text = "Discount IPC moderat (50–62) + block — masih valid."
    else:
        discount_text = "Di luar zona discount IPC ideal."

    text = f"""🟦 SMC IPC INTRADAY SIGNAL — {symbol}

SMC IPC SCORE: {score}/160 — Tier {tier} — {side_label}
...
📌 Checklist IPC (Core Chain)
• Bias 1H strong / tidak bearish : {mark(bias_ok)}
• Struktur 15m bullish           : {mark(struct_ok)}
• Sweep liquidity (core)         : {mark(sweep_ok)}
• CHoCH / impuls (trigger core)  : {mark(choch_ok)}
• Discount IPC (core)            : {mark(disc_core)}
• Deep discount IPC              : {mark(deep_disc)}
• IPC Block (MB / Breaker)       : {mark(block_core)}
• FVG / Imbalance confluence     : {mark(fvg_ok)}
• Liquidity target 15m           : {mark(liq_target_ok)}
• Market kondisi sehat           : {mark(market_ok)}
...
📝 Catatan
Tier:
• A+ : setup IPC sangat kuat (jarang muncul)
• A  : setup IPC kuat, fokus winrate
• B  : setup masih oke tapi kurang confluence
